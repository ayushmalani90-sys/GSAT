...
const TF_AGGREGATION: Record<Timeframe, "15m" | "1h" | "4h" | "1d"> = {
  "15m": "15m",
  "1H": "1h",
  "4H": "4h",
  "1D": "1d",
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      methodology: {
        note: "One normalized BiQuote 1-minute master series is deterministically aggregated into 15m, 1H, 4H and 1D before analysis.",
        indicators: "EMA uses close prices; RSI and ATR use Wilder RMA; MACD uses EMA 12/26 with EMA 9 signal.",
        dataQuality: "A single master series improves cross-timeframe consistency but does not guarantee TradingView parity without the same underlying feed, session boundaries and completed-bar state.",
