...
export type TechnicalAnalysis = {
  ...
  fibonacci: {
    ...
    currentRetracement?: string | null;
    continuationAbove?: number | null;
    bearishConfirmationBelow?: number | null;
    interpretation: string;
  };
  volumeProfile: {
    poc: number | null;
    ...
    interpretation: string;
    position?: "Above POC" | "Below POC" | "At POC" | "Unavailable";
  };
  patterns: Array<{
    name: string;
...
export function supportResistance(candles: TechnicalCandle[], current: number, atrValue = atr(candles, 14)) {
  ...
  for (let i = pivotWindow; i < recent.length - pivotWindow; i += 1) {
    ...
    const avgRange = Math.max(
      recent.slice(Math.max(0, i - pivotWindow), Math.min(recent.length, i + pivotWindow + 1)).reduce((s, x) => s + (x.h - x.l), 0) / (pivotWindow * 2 + 1),
      1e-9,
    );
  ...
  const buildZones = (type: "support" | "resistance") => {
    ...
    for (const candidate of sorted) {
      ...
      if (!nearest) zones.push({ values: [candidate], low: candidate.price - zoneWidth, high: candidate.price + zoneWidth });
        ...
        nearest.values.push(candidate);
        nearest.low = Math.min(nearest.low, candidate.price - zoneWidth);
        nearest.high = Math.max(nearest.high, candidate.price + zoneWidth);
      }
    }
    ...
    return zones
      .map(zone => {
        ...
        const touchScore = Math.min(40, touches * 8);
        const reactionScore = Math.min(20, avgReaction * 10);
        const recencyScore = avgAge * 15;
        const volumeScore = Math.min(10, avgVolume * 4);
        const proximityScore = proximity * 15;
        const strength = Math.max(0, Math.min(100, Math.round(touchScore + reactionScore + recencyScore + volumeScore + proximityScore)));
        return {
          ...
          strength,
          touches,
          distancePct: round(Math.abs(current - center) / Math.abs(current) * 100, 2),
        } satisfies Level;
      })
      .filter(x => x.distancePct >= 0.25)
      .sort((a, b) => b.strength - a.strength || a.distancePct - b.distancePct)
      .filter((zone, index, arr) => index === 0 || arr.slice(0, index).every(prev => Math.abs(prev.price - zone.price) >= minDistance * 0.8))
      .slice(0, 3);
  };

  return {
    supports: buildZones("support"),
    resistances: buildZones("resistance"),
    method: "Structural swing highs/lows using 5-bar pivots, ATR-based filtering, minimum-distance enforcement, zone clustering, touch counts, reaction/recency/volume scoring, top 3 each side.",
  };
}
...
export function volumeProfile(candles: TechnicalCandle[], bins = 24) {
  ...
  recent.forEach(c => {
    ...
    bucketVolume[index] += normalizeVolume(c.v);
  });
...
function sign(value: number | null) {
  return value == null ? 0 : value > 0 ? 1 : value < 0 ? -1 : 0;
}
...
export function analyze(candles: TechnicalCandle[]): TechnicalAnalysis {
  ...
  const bullishCount = [e20, e50, e200].filter(e => price != null && e != null && price > e).length;
  const bearishCount = [e20, e50, e200].filter(e => price != null && e != null && price < e).length;
  const alignmentBull = e20 != null && e50 != null && e200 != null && e20 > e50 && e50 > e200;
  ...
  return {
    ...
    volumeProfile: vp,
    patterns,
    overall: { bias: overallBias, confidence, summary },
  };
}
